...
def main():
    ...
    if args.is_training:
        for ii in range(args.itr):
            ...
            # Move model to GPU
            if args.use_gpu:
                exp.model.to(f'cuda:{args.gpu}')
                if args.use_multi_gpu:
                    exp.model = torch.nn.DataParallel(exp.model, device_ids=args.device_ids)
        ...
        # test에서도 do_predict 추가함 2024.07.30
        print("Do predict는? ", args.do_predict)
        if args.do_predict:
            print('>>>>>>>predicting : {}<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<'.format(setting))
            exp.predict(setting, True)
